...
// GET /health          — lightweight liveness (API only)
// GET /health/full     — deep check: Redis + Postgres + YouTube
...
async function checkRedis(): Promise<{ ok: boolean; latencyMs?: number; error?: string }> {
  ...
  // Use a throw-away queue just to ping — no worker needed
  const q = new Queue('__health-ping__', { connection: redisConnection });
  try {
    await q.client;                          // forces connection
